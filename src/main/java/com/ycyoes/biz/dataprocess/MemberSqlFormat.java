...
public class MemberSqlFormat {
    public static void main(String[] args) {
        ...
        String head = "insert into ZZ_FAMILY_PERSON_202010181600(id, p_id, f_id, relation, create_date, create_user) select sys_guid(), (select id from zz_person where card_num='612525197901025849'), (select f.id from ZZ_FAMILY f inner join zz_person p on f.p_id=p.id where p.card_num='";
        String middle = "'), '";
        ...
        for (int i = 0; i < list.size(); i++) {
            ...
            if (StringUtils.isNotBlank(cardNum) && StringUtils.isNotBlank(relation)) {
                ...
                map.put(cardNum, relation);
//                sb.append(head + cardNum + middle + relation + tail + "\n");
            }
        }
        for (Map.Entry entry : map.entrySet()) {
            sb.append(head + entry.getKey() + middle + entry.getValue() + tail + "\n");
